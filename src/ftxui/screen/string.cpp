...
class SpecicWBPIntervals {
 public:
  template <size_t N>
  SpecicWBPIntervals(const std::array<WordBreakPropertyInterval, N>& table,
                     WBP intervalType) {
    for (const WordBreakPropertyInterval& interval : table) {
      if (interval.property == intervalType) {
        intervals.push_back({interval.first, interval.last});
      }
    }
  }

  const std::vector<Interval>& getTable() const { return intervals; }
...
static const SpecicWBPIntervals g_extend_characters(g_word_break_intervals,
                                                    WBP::Extend);

// Find a codepoint inside a sorted list of Interval.
template <typename TableType>
bool Bisearch(uint32_t ucs, const TableType& table) {
...
bool IsCombining(uint32_t ucs) {
  return Bisearch(ucs, g_extend_characters.getTable());
}
